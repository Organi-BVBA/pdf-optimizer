<template>
  <div class="bg-white px-4 py-3 flex items-center justify-between sm:px-6">
    ...
    >
      <component
        :is="prevPage ? 'a' : 'button'"
        :href="prevPage"
        class="relative inline-flex items-center px-4 py-2 border border-gray-300 text-sm leading-5 font-medium rounded-md text-gray-700 bg-white dark:bg-gray-800 hover:text-gray-500 focus:outline-none focus:shadow-outline-blue focus:border-blue-300 active:bg-gray-100 active:text-gray-700 transition ease-in-out duration-150"
      >
        Previous
      </component>
      <component
        :is="nextPage ? 'a' : 'button'"
        :href="nextPage"
      ...
      <div>
        ...
        >
          <component
            :is="prevPage ? 'a' : 'button'"
            :href="prevPage"
            class="relative inline-flex items-center px-2 py-2 rounded-l-md border border-gray-300 bg-white dark:bg-gray-800 text-sm leading-5 font-medium text-gray-500 hover:text-gray-400 focus:z-10 focus:outline-none focus:border-blue-300 focus:shadow-outline-blue active:bg-gray-100 active:text-gray-500 transition ease-in-out duration-150"
            aria-label="Previous"
          >
            <svg
              class="h-5 w-5"
          ...
          <component
            :is="link.label ? 'a' : 'span'"
            v-for="link in pageLinks"
            :key="link.url"
            :href="link.url"
            class="-ml-px relative inline-flex items-center px-4 py-2 border border-gray-300 bg-white dark:bg-gray-800 text-sm leading-5 font-medium text-gray-700 transition ease-in-out duration-200"
            :class="{
          ...
          <component
            :is="nextPage ? 'a' : 'button'"
            :href="nextPage"
            class="-ml-px relative inline-flex items-center px-2 py-2 rounded-r-md border border-gray-300 bg-white dark:bg-gray-800 text-sm leading-5 font-medium text-gray-500 hover:text-gray-400 focus:z-10 focus:outline-none focus:border-blue-300 focus:shadow-outline-blue active:bg-gray-100 active:text-gray-500 transition ease-in-out duration-150"
            aria-label="Next"
          >
            <svg
              class="h-5 w-5"
...
export default defineComponent( {
  ...
  computed: {
    pageLinks () {
      ...
      const pageRange = this.range( 1, this.meta.last_page + 1 ).map( ( page ) => {
        parsed.page = page;
      ...
      return pageRange.length > pageLimit ? this.trimPageRange( pageRange ) : pageRange;
    },

    prevPage () {
      const parsed = qs.parse( window.location.search.substr( 1 ) );
      parsed.page = parsed.page ? parsed.page : 1;
